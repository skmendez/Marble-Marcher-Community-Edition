...
//!
//! Unlike the first (now-reverted, see `render::FineRenderTarget`'s doc)
//! attempt at this, the decision here writes directly into
//! `render::FineRenderTarget::active_size` -- a cheap `Camera.viewport`
//! sub-rectangle mutation, not a GPU render-target rebuild. That's the
//! whole reason this retry is safe: the hysteresis logic below is carried
//! over unchanged from the reverted `present.rs`-driving version (it was
//! never the problem), only its *consumer* changed.

use bevy::prelude::*;

use crate::fps_overlay::FrameTimeWindow;
use crate::render::FineRenderTarget;
...
/// reaching the clamp range within a handful of adjustment intervals.
const SCALE_STEP_DOWN: f32 = 0.95;
const SCALE_STEP_UP: f32 = 1.05;

/// How often (seconds) the controller re-evaluates the scale.
///
/// The reverted first attempt used 1.5s here, but that value was tuned
/// around a since-eliminated cost: every *actual* scale change back then
/// rebuilt the render target as a whole new GPU `Image` (a heavy
/// operation), so the interval had to be coarse enough to keep the number
/// of real resizes low. That consumer no longer exists -- an `active_size`
/// write today is a plain `Camera.viewport`/`Transform.scale`/present-UV
/// component write with **no GPU resource allocation** (`FineRenderTarget`'s
/// doc), the exact same cheap path `oscillate_fine_resolution_tier` has
/// already exercised continuously, every frame, for this entire session's
/// testing with no reported jitter. There is no longer a resize-cost reason
/// to throttle coarsely.
///
/// 0.5s (down from 1.5s) was chosen as a starting point -- still far
/// coarser than per-frame (so a single-frame frame-time spike can't cause a
/// visible step; `FrameTimeWindow`'s own ~1s rolling average already smooths
/// that further), but responsive enough to converge in a couple of seconds
/// under sustained load rather than several. Confirmed smooth (no visible
/// jitter/flicker, gradual step-by-step convergence) under live sustained
/// GPU load via the CDP smoothness check described in this feature's
/// landing notes -- see the commit that introduced this constant.
pub const ADJUST_INTERVAL_SECS: f64 = 0.5;
...
/// dimension at 1px (a 0-sized `Image`/texture is invalid).
...
/// `Update` system, gated on `?adaptive_res=1`/`MM_ADAPTIVE_RES=1`:
/// re-evaluates `AdaptiveResolution::scale` from the live `FrameTimeWindow`
/// average, throttled to `ADJUST_INTERVAL_SECS`, and writes the resulting
/// pixel size into `FineRenderTarget::active_size` -- the same field
/// `oscillate_fine_resolution_tier` drives, just computed from a real
/// frame-time signal instead of a timer sweep.
///
/// Mutually exclusive with `oscillate_fine_resolution_tier`: both write
/// `active_size`, and if both were somehow enabled at once the oscillator's
/// always-visible manual smoothness sweep would fight this system's
/// load-driven adjustment every frame. `res_oscillate_enabled` wins that
/// conflict (this system just early-returns) since it's the narrower,
/// explicitly-requested manual test tool -- a caller who asks for it wants
/// to *see* the sweep, not have it silently overridden.
pub fn adjust_resolution_scale(
    ...
    if !config.adaptive_res_enabled || config.res_oscillate_enabled {
        return;
    }
    ...
    let target = target_pixel_size(render_target.max_size, adaptive.scale);
